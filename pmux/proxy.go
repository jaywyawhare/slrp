...
func NewProxy(addr string, t string) Proxy {
    ...
    ipAddr := addrPort.Addr().Unmap()
    if !ipAddr.Is4() {
        return 0
    }

    b := ipAddr.As4()
    ipv4u64 := (uint64(b[0]) << 24) | (uint64(b[1]) << 16) | (uint64(b[2]) << 8) | uint64(b[3])
    ipBits := ipv4u64 << 32
    portBits := uint64(addrPort.Port()) << 16
    return Proxy(ipBits + portBits + uint64(p))
}
